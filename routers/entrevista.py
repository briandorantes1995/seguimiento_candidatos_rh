...
from db.models import Candidato, Entrevista, EntrevistaUpdate, EntrevistaCreate, Usuario, get_session
...
def entrevistas_por_postulacion(postulacion_id: int, request: Request, session: Session = Depends(get_session), current_user: Usuario = Depends(get_current_user)):
    ...
    return templates.TemplateResponse(request=request, name="candidato.html", context={"postulacion": postulacion, "postulacion_id": postulacion_id})
...
def get_entrevista(entrevista_id: int, request: Request, session: Session = Depends(get_session), current_user: Usuario = Depends(get_current_user)):
...
def update_entrevista(entrevista_id: int, entrevista_data: EntrevistaUpdate, request: Request, session: Session = Depends(get_session), current_user: Usuario = Depends(get_current_user)):
    entrevista = session.get(Entrevista, entrevista_id)
    if not entrevista:
        raise HTTPException(status_code=404)
    check_owner(entrevista, current_user)
...
def delete_entrevista(entrevista_id: int, request: Request, session: Session = Depends(get_session), current_user: Usuario = Depends(get_current_user)):
    entrevista = session.get(Entrevista, entrevista_id)
    if not entrevista:
        raise HTTPException(status_code=404)
    check_owner(entrevista, current_user)
